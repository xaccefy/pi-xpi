...
import { type TUI, truncateToWidth, Text } from "@earendil-works/pi-tui";
...
export const TodoParamsSchema = Type.Object({
	...
	activeForm: Type.Optional(Type.String({ description: "Present-continuous spinner label shown while status is in_progress" })),
	status: Type.Optional(
		Type.Union([Type.Literal("pending"), Type.Literal("in_progress"), Type.Literal("completed"), Type.Literal("deleted")], {
	...
	includeDeleted: Type.Optional(Type.Boolean({ description: "If true, list returns deleted tasks too" }))
});
...
function hasCycle(tasks: Task[], taskId: number, newBlockedBy: number[]): boolean {
	...
	for (const t of tasks) {
		adj.set(t.id, t.id === taskId ? [...(t.blockedBy ?? []), ...newBlockedBy] : [...(t.blockedBy ?? [])]);
	}
	...
	switch (action) {
		...
		case "update": {
			...
			const hasMutation = params.subject !== undefined || params.description !== undefined ||
				params.activeForm !== undefined || params.status !== undefined ||
				params.owner !== undefined || params.metadata !== undefined ||
		...
		case "list": {
			...
			const formatted = view.length === 0 ? "No tasks" : view.map((t) => {
...
// ---------------------------------------------------------------------------
const STATUS_GLYPH: Record<TaskStatus, string> = { pending: "○", in_progress: "◐", completed: "●", deleted: "⊘" };
const STATUS_COLOR: Record<TaskStatus, "dim" | "warning" | "success" | "muted"> = { pending: "dim", in_progress: "warning", completed: "success", deleted: "muted" };
...
function formatOverlayTaskLine(t: Task, theme: Theme, showId: boolean): string {
	const glyph = t.status === "pending" ? theme.fg("dim", "○") :
	...
	line += ` ${subject}`;
	if (t.status === "in_progress" && t.activeForm) line += ` ${theme.fg("dim", `(${t.activeForm})`)}`;
	if (t.blockedBy?.length) line += ` ${theme.fg("dim", `⛓ ${t.blockedBy.map((id) => `#${id}`).join(",")}`)}`;
	return line;
}
...
export class TodoOverlay {
	...
	update(): void {
		...
		const visible = snapshot.tasks.filter((t) => t.status !== "deleted" && !(t.status === "completed" && this.hiddenCompletedTaskIds.has(t.id)));
	...
	private renderWidget(theme: Theme, width: number): string[] {
		...
		const completedSet = new Set(state.tasks.filter((t) => t.status === "completed").map((t) => t.id));
		for (const id of this.completedTaskIdsPendingHide) if (!completedSet.has(id)) this.completedTaskIdsPendingHide.delete(id);
		for (const id of this.hiddenCompletedTaskIds) if (!completedSet.has(id)) this.hiddenCompletedTaskIds.delete(id);

		const overlayTasks = state.tasks.filter((t) => t.status !== "deleted" && !(t.status === "completed" && this.hiddenCompletedTaskIds.has(t.id)));
		...
		const hasActive = overlayTasks.some((t) => t.status === "in_progress" || t.status === "pending");
		const showIds = overlayTasks.some((t) => t.blockedBy && t.blockedBy.length > 0);

		const headingColor = hasActive ? "accent" : "dim";
		const headingIcon = hasActive ? "●" : "○";
		const heading = truncate(`${theme.fg(headingColor, headingIcon)} ${theme.fg(headingColor, `Todos (${completed}/${total})`)}`);
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		renderResult(result, _opts, theme, _context) {
			...
			if (details) {
				...
				else if (details.action === "update") status = p.status ?? details.tasks.find((t) => t.id === p.id)?.status;
				else if (details.action === "delete") status = details.tasks.find((t) => t.id === p.id)?.status;
			}
			if (status) return new Text(theme.fg(STATUS_COLOR[status], `${STATUS_GLYPH[status]} ${formatStatusLabel(status)}`), 0, 0);
	...
	pi.on("session_shutdown", async (_event, ctx) => {
		let id = "";
		try { id = sid(ctx); } catch (e) { if (!/stale after session replacement/.test(String(e))) throw e; }
		sessions.delete(id);
		if (id === "" || id === activeRenderSession) {
			try { todoOverlay?.dispose(); } finally {
