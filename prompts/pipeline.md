...
- **audit** — Call `Agent` with `subagent_type: "auditor"` to scan for the requested vulnerability class / reachability path. Review candidates; open `CaseAdd(status: hypothesis)` per plausible one (dedupe via `CaseList` first).
